...
//     match cfg.cmd {
//         cmd::Cmd::Report(report_cfg) => cmd::report_old::report(&db, report_cfg.to_cfg().await)
//             .await
...
//     cmd::requirements_old::collect(db, &collect_file.requirements)
//         .await
//         .map_err(MantraError::Extract)?;

//     cmd::trace_old::collect(db, &collect_file.traces)
//         .await
//         .map_err(MantraError::Trace)?;

//     if let Some(coverage) = collect_file.coverage {
//         for file in coverage.files {
//             let coverage_changes = cmd::coverage_old::collect_from_path(db, &file)
//                 .await
...
//         let added_review_cnt = cmd::review_old::collect(db, review)
//             .await
