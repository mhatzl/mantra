...
select distinct r.id from Requirements r, DirectReqTraces tr
...
from IndirectlyTracedRequirements ir, RequirementDescendants c, DirectReqTraces t
...
-- from DirectReqTraces rt, Items i, TestItems ti, TracedItems tr
-- where 
...
select distinct dt.req_id from DirectReqTraces dt, DirectTracedItems dti, Items i, TestRunStatementCoverage trc, TestStatementCoverage tc
where dt.line = dti.traced_line and dt.filepath = dti.filepath and dti.filepath = i.filepath and dti.item_start_line = i.start_line
 and (
    (i.filepath = trc.stmnt_filepath and i.start_line <= trc.stmt_line and i.end_line >= trc.stmt_line)
    or (i.filepath = tc.stmnt_filepath and i.start_line <= tc.stmt_line and i.end_line >= tc.stmt_line));
